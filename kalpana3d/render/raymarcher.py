...
# Scene definition (hardcoded for now)
@njit(fastmath=True)
def scene_sdf(p):
    """
    Defines the scene by combining SDFs. For now, it's just a single sphere.
    """
...
@njit(fastmath=True)
def ray_march(ro, rd, max_steps=100, min_dist=0.001, max_dist=100.0):
    """
...
def render(width, height, camera_pos, light_pos):
    """
